...
from app_functions.llm_agents import openai_summarizer, groq_summarizer, openai_NERextractor
company_symbol_map = {
  "apple": "aapl",
  "microsoft": "msft",
  "alphabet": "googl",
...
def plot_stock_data(ticker, data):
    ...
    layout = go.Layout(title=f"{ticker} Stock Data")
...
user_input = st.text_input("Enter your query for stock analysis")

# Fetch and display financial data when the user clicks the button
if st.button("Get Summary 📊"):
    if user_input:
        info = openai_NERextractor(user_input)
        data = json.loads(info)
        company = data["Company"]
        month = data["time"]["month"]
        year = data["time"]["year"]
        ticker = company_symbol_map[company.lower()]
        data, news, stock_data = financial_data(ticker, year, month)
